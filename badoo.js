window.opener = localStorage.getItem("opener") || "Привет! Познакомимся? ;)";
...
window.bot = {
	...
	massChat: function(){
		bot.getUsers().then(function(data){
			users = data.filter(function(user){return user.online_status_text != "online_status_text" && user.online_status_text != "Была онлайн больше недели назад" && user.online_status_text != " Was online more than a week ago"});
